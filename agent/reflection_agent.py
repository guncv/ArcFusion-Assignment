...
class ReflectionAgent:
    """
    Agent responsible for evaluating the quality and sufficiency of synthesized answers.

    This agent acts as a quality gate, determining if the generated answer:
    - Fully addresses the user's query
    - Has proper citations and evidence
    - Is clear, coherent, and well-structured
    - Meets quality standards

    If the answer is insufficient, it triggers a retry with improvement suggestions.
    """
    ...
    async def invoke(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            logger.info(
                f"[ReflectionAgent] Evaluation complete: "
            ...
            if not reflection_result.is_sufficient:
                logger.info(
                    f"[ReflectionAgent] Issues found: {', '.join(reflection_result.issues)}"
                )
                logger.info(
                    f"[ReflectionAgent] Suggestions: {', '.join(reflection_result.suggestions)}"
                )
            ...
            logger.error(f"[ReflectionAgent] Error during reflection: {e}", exc_info=True)

            # Default to accepting the answer if reflection fails
            logger.warning("[ReflectionAgent] Reflection failed, accepting current answer")
            return {
                **state,
    ...
    def _extract_reflection_result(self, response: Any) -> ReflectionResult:
        ...
        try:
            ...
            logger.warning("[ReflectionAgent] No tool call found in response, accepting answer")
            return ReflectionResult(
                is_sufficient=True,
                quality_score=0.7,
                issues=[],
            ...
            logger.error(f"[ReflectionAgent] Error extracting result: {e}", exc_info=True)
